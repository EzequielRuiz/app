        ...
        .then(res => {
           if (res.data.status == "success") {
            this.$notify({
              ...
              message: "Exito, Alarma Eliminada!!"
            });
        ...
        .then(res => {
          if (res.data.status == "success") {
            this.$notify({
              type: "success",
              icon: "tim-icons icon-check-2",
              message: "Alarma Creada con EXITO!!"
            });
    ...
    createNewRule() {
      if (this.selectedWidgetIndex == null) {
        this.$notify({
          type: "warning",
          icon: "tim-icons icon-alert-circle-exc",
          message: " Una Variable debe ser Seleccionada"
        });
        return;
      }
      ...
      if (this.newRule.condition == null) {
        this.$notify({
          type: "warning",
          icon: "tim-icons icon-alert-circle-exc",
          message: " Una Condicion debe ser Seleccionada"
        });
        return;
      }
      ...
      if (this.newRule.value == null) {
        this.$notify({
          type: "warning",
          icon: "tim-icons icon-alert-circle-exc",
          message: " La VARIABLE está Vacia"
        });
        return;
      }
